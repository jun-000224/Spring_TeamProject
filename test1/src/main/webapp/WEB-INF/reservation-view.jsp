...
<head>
  ...
  <style>
    ...
    .poi-item p { margin:0; line-height:1.6; } .poi-item p:first-child strong { font-size:1.05rem; color:#2c3e50; }
    .save-button-wrap { display:flex; justify-content:center; gap:8px; margin-top:26px; flex-wrap:wrap; } .save-button-wrap .btn { min-width:140px; }
    ...
    .memo-field { width:100%; min-width:200px; max-width:100%; min-height:160px; padding:12px 14px; border:1px solid var(--border); border-radius:10px; background:#fff; font-size:.95rem; line-height:1.6; color:var(--text); resize:both; box-sizing:border-box; outline:none; transition:border-color .12s ease, box-shadow .12s ease; }
    ...
    <div id="app">
      ...
      <div class="panel">
        ...
        <div class="memo-card" aria-label="메모 영역">
          ...
          <textarea class="memo-field" rows="5" v-model="memo" placeholder="여행 메모를 입력하세요. (예: 예산 유의 사항, 요청사항 등)"></textarea>
          <div class="memo-hint">저장하기를 누르면 메모가 DB의 <code>RESERVATION.DESCRIPT</code> 칼럼에 저장됩니다.</div>
        </div>
      </div>
      ...
      <div class="save-button-wrap">
        <button class="btn primary" @click="fnSave">저장하기</button>
...
<script>
  const app = Vue.createApp({
    ...
    methods: {
      ...
      initializeMap(data) {
        if (!window.kakao || !kakao.maps) {
          document.getElementById('map-container').innerText = 'Kakao Map API 로드 실패.'; return;
        }
      ...
      drawPolyline(points) {
        ...
        this.routePolyline = new kakao.maps.Polyline({ path: path, strokeWeight: 5, strokeOpacity: 0.9 });
      ...
      // 저장하기: 코스명 + 메모(descript) 함께 전송 (POST 필수)
      fnSave() {
        let self = this;
        ...
        saveTitleAndMemo.always(() => {
          alert("저장에 성공하였습니다! 나의 예약 페이지로 이동합니다.");
          window.location.href = '/myReservation.do?resNum=' + this.reservation.resNum;
        });
      },
    ...
    mounted() {
      ...
      if (this.reservation.descript) {
        this.memo = this.reservation.descript;
      }
