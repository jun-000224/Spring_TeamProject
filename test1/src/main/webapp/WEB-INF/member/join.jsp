  ...
  <div class="container">
    <div class="card">
      ...
      <div class="grid grid-2">
        <div class="field">
          ...
          <div class="row-inline">
            ...
            <input v-else type="text" class="input grow" v-model="id" disabled>
      ...
      <div class="grid">
        ...
        <div class="field" v-if="!certifiFlg">
      ...
      <div class="actions">
        ...
        <button type="button" class="btn primary" @click="fnJoin">가입하기</button>
      </div>
    </div>
  </div>
  ...
  const app = Vue.createApp({
    data(){
      return {
        ...
        timer:"", count:180,
        smsFlg:false, certifiFlg:false, inputNum:"", certifiStr:"",
    ...
    methods:{
      ...
      fnIdCheck(){
        ...
        $.ajax({
          url:"/member/idCheck.dox", type:"POST", dataType:"json", data:param,
          success:(data)=>{
            if(data.result=='false'){
              alert(data.msg); this.idFlg = true;
            }else{
              alert(data.msg);
            }
          }
      ...
      fnAddr(){ window.open("/member/addr.do","addr","width=500,height=500"); },
      fnResult(roadFullAddr){ this.addr = roadFullAddr; },

      fnSms(){
        $.ajax({
          url:"/send-one", type:"POST", dataType:"json", data:{},
          success:(data)=>{
      ...
      fnJoin(){
        ...
        if(!this.idFlg) { this.showAlert("아이디 중복체크를 완료해주세요."); return; }
        if(this.pwd !== this.pwd2){ this.showAlert("비밀번호가 일치하지 않습니다."); return; }
        if(this.name === ""){ this.showAlert("이름을 입력해주세요."); return; }
        if(this.emailFront === ""){ this.showAlert("이메일을 입력해주세요."); return; }
        if(this.emailBack === "abc"){ this.showAlert("이메일 도메인을 선택해주세요."); return; }
        if(this.addr === ""){ this.showAlert("주소를 입력해주세요."); return; }
        if(this.phone2.length !== 4 || this.phone3.length !== 4){ this.showAlert("전화번호를 정확히 입력해주세요."); return; }
        if(this.nickname === ""){ this.nickname = this.name; }
        // if(!this.certifiFlg){ this.showAlert("전화번호를 인증해주세요."); return; }
